...
function kidazzle_render_general_seo_meta_box($post)
{
    ...
    <div class="KIDazzle-seo-field" style="margin-bottom: 15px;">
        <label for="meta_description" style="display: block; font-weight: 600; margin-bottom: 5px;">
            <?php _e('Meta Description', 'kidazzle'); ?>
        </label>
        <textarea id="meta_description" name="meta_description" rows="4" style="width: 100%;"
            placeholder="<?php _e('Enter custom meta description...', 'kidazzle'); ?>"><?php echo esc_textarea($meta_description); ?></textarea>
        <p class="description" style="font-size: 12px; margin-top: 5px;">
            <?php _e('Leave empty to use the auto-generated description.', 'kidazzle'); ?>
        </p>
    </div>

    <div class="KIDazzle-seo-field">
        <label for="meta_keywords" style="display: block; font-weight: 600; margin-bottom: 5px;">
            <?php _e('Meta Keywords', 'kidazzle'); ?>
        </label>
        <textarea id="meta_keywords" name="meta_keywords" rows="2" style="width: 100%;"
            placeholder="<?php _e('keyword1, keyword2, keyword3', 'kidazzle'); ?>"><?php echo esc_textarea($meta_keywords); ?></textarea>
        <p class="description" style="font-size: 12px; margin-top: 5px;">
            <?php _e('Comma-separated list. Leave empty to use auto-generated keywords.', 'kidazzle'); ?>
        </p>
    </div>
    </div>
    ...
    <div style="margin-top: 15px; border-top: 1px solid #eee; padding-top: 15px;">
        <button type="button" id="KIDazzle-seo-autofill" class="button">
            ...
            <?php _e('Auto-Fill with AI', 'kidazzle'); ?>
        </button>
        <span class="spinner" id="KIDazzle-seo-spinner" style="float: none; margin: 0 5px;"></span>
        <p class="description" style="margin-top: 5px;">
            <?php _e('Generates a description and keywords based on page content.', 'kidazzle'); ?>
        </p>
    </div>
    ...
    <script>
        jQuery(document).ready(function ($) {
            $('#KIDazzle-seo-autofill').on('click', function (e) {
                ...
                if (!confirm('<?php _e('This will overwrite existing SEO fields with AI-generated content. Continue?', 'kidazzle'); ?>')) {
                    return;
                }
