...
{
	$labels = array(
		'name' => _x('Programs', 'Post Type General Name', 'kidazzle'),
		'singular_name' => _x('Program', 'Post Type Singular Name', 'kidazzle'),
		'menu_name' => __('Programs', 'kidazzle'),
		'all_items' => __('All Programs', 'kidazzle'),
		'add_new_item' => __('Add New Program', 'kidazzle'),
		'edit_item' => __('Edit Program', 'kidazzle'),
		'view_item' => __('View Program', 'kidazzle'),
	);

	$program_slug = kidazzle_get_program_base_slug();

	$args = array(
		'label' => __('Program', 'kidazzle'),
		'labels' => $labels,
...
function kidazzle_program_admin_columns($columns)
{
	...
	$new_columns['age_range'] = __('Age Range', 'kidazzle');
	$new_columns['locations'] = __('Locations', 'kidazzle');
...
function kidazzle_program_title_placeholder($title)
{
	...
	if ('program' === $screen->post_type) {
		$title = __('e.g., Infant Care', 'kidazzle');
	}
	return $title;
}
...
function kidazzle_program_meta_box()
{
	add_meta_box(
		...
		__('Program Anchor & SEO Intro', 'kidazzle'),
...
function kidazzle_program_meta_box_render($post)
{
	...
	<p>
		<label for="program_anchor_slug"
			class="screen-reader-text"><?php esc_html_e('Program Anchor', 'kidazzle'); ?></label>
		<input type="text" id="program_anchor_slug" name="program_anchor_slug" value="<?php echo esc_attr($anchor); ?>"
			class="widefat" placeholder="<?php esc_attr_e('e.g., infant', 'kidazzle'); ?>" />
		<small><?php esc_html_e('Used for #anchors and homepage wizard links. Defaults to the slug.', 'kidazzle'); ?></small>
	</p>
	<p>
		<label for="program_seo_heading"
			class="screen-reader-text"><?php esc_html_e('SEO Heading', 'kidazzle'); ?></label>
		<input type="text" id="program_seo_heading" name="program_seo_heading" value="<?php echo esc_attr($heading); ?>"
			class="widefat" placeholder="<?php esc_attr_e('Program intro heading', 'kidazzle'); ?>" />
	</p>
	<p>
		<label for="program_seo_summary"
			class="screen-reader-text"><?php esc_html_e('SEO Summary', 'kidazzle'); ?></label>
		<textarea id="program_seo_summary" name="program_seo_summary" class="widefat" rows="3"
			placeholder="<?php esc_attr_e('Short overview that lives above the card', 'kidazzle'); ?>"><?php echo esc_textarea($summary); ?></textarea>
	</p>
	<p>
		<label for="program_seo_highlights"
			class="screen-reader-text"><?php esc_html_e('SEO Highlights', 'kidazzle'); ?></label>
		<textarea id="program_seo_highlights" name="program_seo_highlights" class="widefat" rows="4"
			placeholder="<?php esc_attr_e("One bullet per line (e.g. ratios, curriculum)", 'kidazzle'); ?>"><?php echo esc_textarea($highlights); ?></textarea>
	</p>
	<hr />
	<p>
		<label for="program_meta_title"
			class="screen-reader-text"><?php esc_html_e('Meta Title', 'kidazzle'); ?></label>
		<input type="text" id="program_meta_title" name="program_meta_title" value="<?php echo esc_attr($meta_title); ?>"
			class="widefat" placeholder="<?php esc_attr_e('Custom title tag (optional)', 'kidazzle'); ?>" />
		<small><?php esc_html_e('Used on the program detail for search visibility.', 'kidazzle'); ?></small>
	</p>
	<p>
		<label for="program_meta_description"
			class="screen-reader-text"><?php esc_html_e('Meta Description', 'kidazzle'); ?></label>
		<textarea id="program_meta_description" name="program_meta_description" class="widefat" rows="3"
			placeholder="<?php esc_attr_e('1–2 sentence description for search snippets', 'kidazzle'); ?>"><?php echo esc_textarea($meta_desc); ?></textarea>
	</p>
	<p>
		<label for="program_faq_items"
			class="screen-reader-text"><?php esc_html_e('FAQ Items', 'kidazzle'); ?></label>
		<textarea id="program_faq_items" name="program_faq_items" class="widefat" rows="4"
			placeholder="<?php esc_attr_e('Question | Answer (one per line)', 'kidazzle'); ?>"><?php echo esc_textarea($faq_items); ?></textarea>
		<small><?php esc_html_e('Populate FAQ schema and on-page Q&A.', 'kidazzle'); ?></small>
	</p>
	<?php
}
...
function kidazzle_program_locations_meta_box()
{
	add_meta_box(
		...
		__('Available at Locations', 'kidazzle'),
	...
	add_meta_box(
		...
		__('Single Page Content', 'kidazzle'),
...
function kidazzle_program_locations_meta_box_render($post)
{
	...
	?>
	<p><?php _e('Select the locations where this program is available:', 'kidazzle'); ?></p>
	<p style="margin-bottom: 10px;">
		<button type="button" id="KIDazzle-toggle-all-locations" class="button button-secondary" style="margin-bottom: 5px;">
			<?php _e('Select All / Deselect All', 'kidazzle'); ?>
		</button>
	</p>
	<div id="KIDazzle-locations-list"
		...
		<?php if (!empty($all_locations)): ?>
			...
			<?php endforeach; ?>
		<?php else: ?>
			<p><?php _e('No locations found. Please add locations first.', 'kidazzle'); ?></p>
		<?php endif; ?>
	</div>
	<p><small><?php _e('This program will only appear on selected location pages.', 'kidazzle'); ?></small></p>
...
function kidazzle_program_details_meta_box_render($post)
{
	...
	<div class="KIDazzle-program-field">
		<label for="program_icon"><?php _e('Program Icon/Emoji', 'kidazzle'); ?></label>
		...
		<small><?php _e('Select a preset or type a custom emoji.', 'kidazzle'); ?></small>
	...
	<div class="KIDazzle-program-field">
		<label for="program_age_range"><?php _e('Age Range', 'kidazzle'); ?></label>
		...
		<small><?php _e('Age range badge shown on program card (e.g., "6w - 12mo", "1 Year", "4yr - 5yr")', 'kidazzle'); ?></small>
	</div>

	<div class="KIDazzle-program-field">
		<label for="program_features"><?php _e('Program Features', 'kidazzle'); ?></label>
		...
		<small><?php _e('Enter one feature per line. These will display with checkmarks on the program card.', 'kidazzle'); ?></small>
	</div>

	<div class="KIDazzle-program-field">
		<label for="program_cta_text"><?php _e('CTA Button Text', 'kidazzle'); ?></label>
		...
		<small><?php _e('Text for the call-to-action button (default: "Schedule Tour")', 'kidazzle'); ?></small>
	</div>

	<div class="KIDazzle-program-field">
		<label for="program_cta_link"><?php _e('CTA Button Link', 'kidazzle'); ?></label>
		<input type="text" id="program_cta_link" name="program_cta_link" value="<?php echo esc_attr($cta_link); ?>"
			placeholder="#tour" />
		<small><?php _e('URL or anchor link for the CTA button (default: "#tour")', 'kidazzle'); ?></small>
	</div>

	<div class="KIDazzle-program-field">
		<label for="program_lesson_plan_file"><?php _e('Lesson Plan File (URL)', 'kidazzle'); ?></label>
		...
		<small><?php _e('Paste the full URL to the PDF lesson plan. If set, a button will appear on the program page.', 'kidazzle'); ?></small>
	</div>

	<div class="KIDazzle-program-field">
		<label for="program_color_scheme"><?php _e('Color Scheme', 'kidazzle'); ?></label>
		...
		<small><?php _e('Color theme for the program card hover effects and badges', 'kidazzle'); ?></small>
	</div>

	<p><strong><?php _e('Note:', 'kidazzle'); ?></strong>
		<?php _e('The program description shown on the card comes from the "Excerpt" field. The featured image is used as the card image.', 'kidazzle'); ?>
	</p>
	<?php
}
...
function kidazzle_program_single_page_meta_box_render($post)
{
	...
	<div class="KIDazzle-single-field">
		<label for="program_hero_title"><?php _e('Hero Title', 'kidazzle'); ?></label>
		...
		<small><?php _e('Main heading on single program page (defaults to program title if empty)', 'kidazzle'); ?></small>
	</div>

	<div class="KIDazzle-single-field">
		<label for="program_hero_description"><?php _e('Hero Description', 'kidazzle'); ?></label>
		...
		<small><?php _e('Description paragraph in hero section', 'kidazzle'); ?></small>
	</div>
	...
	<div class="KIDazzle-single-field">
		<label for="program_prism_title"><?php _e('Prism Section Title', 'kidazzle'); ?></label>
		...
		<small><?php _e('Title for the Prismpath focus section', 'kidazzle'); ?></small>
	</div>

	<div class="KIDazzle-single-field">
		<label for="program_prism_description"><?php _e('Prism Description', 'kidazzle'); ?></label>
		...
		<small><?php _e('Description explaining the program\'s Prismpath focus', 'kidazzle'); ?></small>
	</div>

	<div class="KIDazzle-single-field">
		<label><?php _e('Prismpath Chart Values (0-100)', 'kidazzle'); ?></label>
		...
		<small><?php _e('Set values 0-100 for each pillar. These create the radar chart.', 'kidazzle'); ?></small>
	...
	<div class="KIDazzle-single-field">
		<label for="program_prism_focus_items"><?php _e('Focus Items', 'kidazzle'); ?></label>
		...
		<small><?php _e('Bullet points explaining the focus. One per line.', 'kidazzle'); ?></small>
	</div>
	...
	<div class="KIDazzle-single-field">
		<label for="program_schedule_title"><?php _e('Schedule Section Title', 'kidazzle'); ?></label>
		...
		<small><?php _e('Title for the schedule section', 'kidazzle'); ?></small>
	</div>

	<div class="KIDazzle-single-field">
		<label for="program_schedule_items"><?php _e('Schedule Items', 'kidazzle'); ?></label>
		...
		<small><?php _e('Format: Badge|Title|Description (one per line). Badge can be AM, Mid, PM, or any text.', 'kidazzle'); ?></small>
	</div>

	<p><strong><?php _e('Note:', 'kidazzle'); ?></strong>
		<?php _e('The featured image is used as the hero image on the single program page.', 'kidazzle'); ?></p>
	<?php
}
