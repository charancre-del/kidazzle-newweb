...
class wimper_Link_Equity_Analyzer
{
    ...
    private static function find_suggested_sources($target, $all_pages)
    {
        ...
        foreach ($all_pages as $page) {
            ...
            if (isset($GLOBALS['wimper_link_scores'][$page->ID])) {
                $relevance += min(0.2, $GLOBALS['wimper_link_scores'][$page->ID] / 50);
            }
    ...
    public function render_dashboard()
    {
        ...
        $nonce = wp_create_nonce('wimper_link_equity');
        ?>
        <div class="wrap">
                ...
                <table class="wp-list-table widefat fixed striped" id="recommendations-table">
                    ...
                    <tbody>
                        <?php foreach (array_slice($recommendations, 0, 30) as $rec): ?>
                            <tr data-post-id="<?php echo $rec['post_id']; ?>" data-title="<?php echo esc_attr($rec['title']); ?>"
                                    ...
                                    <?php if (!empty($rec['suggested_sources'])): ?>
                                        <div class="suggested-sources">
                                            <?php foreach (array_slice($rec['suggested_sources'], 0, 3) as $src): ?>
                                                <div class="source-item" data-source-id="<?php echo $src['id']; ?>"
        ...
        <div id="ai-fix-modal"
            style="display: none; position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.6); z-index: 100000;">
            <div
                style="background: #fff; max-width: 700px; margin: 40px auto; padding: 25px; border-radius: 8px; max-height: 85vh; overflow-y: auto;">
                ...
                <div style="margin: 20px 0;">
                    ...
                    <div id="ai-fix-link-preview"
                        style="background: #f5f5f5; padding: 10px; border-radius: 4px; margin-top: 5px; font-family: monospace;">
                    </div>
                </div>

                <div style="margin: 20px 0;">
                    <label><strong>Preview (AI will find natural placement):</strong></label>
                    <div id="ai-fix-preview"
                        style="background: #fffbe6; padding: 15px; border: 1px solid #ffe58f; border-radius: 4px; margin-top: 5px; min-height: 100px;">
        ...
        <script>
            jQuery(function ($) {
                ...
                $('#ai-fix-preview-btn').on('click', function () {
                    ...
                    $.post(ajaxurl, {
                        action: 'wimper_link_equity_ai_preview',
                        nonce: nonce,
                        source_id: sourceId,
                ...
                $('#ai-fix-apply-btn').on('click', function () {
                    ...
                    $.post(ajaxurl, {
                        action: 'wimper_link_equity_ai_apply',
                        nonce: nonce,
                        source_id: sourceId,
                        target_id: currentTarget.id,
                        target_title: currentTarget.title,
...
add_action('wp_ajax_wimper_link_equity_ai_preview', function () {
    check_ajax_referer('wimper_link_equity', 'nonce');
...
Find a natural place in the content where this link fits contextually. Return the modified paragraph with the link inserted. Return ONLY the HTML paragraph, no explanation.";
    ...
    $response = $wimper_llm_client->make_request([
        'model' => get_option('wimper_llm_model', 'gpt-4o-mini'),
        'messages' => [
...
add_action('wp_ajax_wimper_link_equity_ai_apply', function () {
    check_ajax_referer('wimper_link_equity', 'nonce');
...
new wimper_Link_Equity_Analyzer();
