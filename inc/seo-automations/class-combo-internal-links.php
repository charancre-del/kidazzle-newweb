...
class wimper_Combo_Internal_Links
{
    public function __construct()
    {
        // Add combo links to program single pages
        add_action('wimper_after_program_content', [$this, 'render_program_city_links']);

        // Add combo links to city pages
        add_action('wimper_after_city_content', [$this, 'render_city_program_links']);
    ...
    public function render_program_city_links()
    {
        ...
        $cities = wimper_Combo_Page_Generator::get_all_cities();
        if (empty($cities))
            return;

        ?>
        <section class="program-city-links py-12 bg-wimper-cream">
            <div class="max-w-6xl mx-auto px-4">
                <h2 class="text-2xl font-serif font-bold text-wimper-ink mb-6">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                    <?php foreach (array_slice($cities, 0, 12) as $city):
                        ...
                        <a href="<?php echo esc_url($url); ?>"
                            class="block p-4 bg-white rounded-lg shadow-sm hover:shadow-md transition border border-wimper-ink/5">
                            <span class="text-sm font-semibold text-wimper-ink">
            ...
            <div class="max-w-6xl mx-auto px-4">
                ...
                <div class="grid md:grid-cols-3 gap-4">
                    <?php foreach ($programs as $program):
                        ...
                        ?>
                        <a href="<?php echo esc_url($url); ?>"
                            class="block bg-wimper-cream rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition group">
                            <?php if ($image): ?>
                                <div class="h-32 overflow-hidden">
                                    <img src="<?php echo esc_url($image); ?>" alt="<?php echo esc_attr($program->post_title); ?>"
                                        class="w-full h-full object-cover group-hover:scale-105 transition-transform">
                                </div>
                            <?php endif; ?>
                            <div class="p-4">
                                <h3 class="font-bold text-wimper-ink"><?php echo esc_html($program->post_title); ?></h3>
                                <p class="text-sm text-wimper-ink/60 mt-1">in <?php echo esc_html($city_name); ?></p>
                            </div>
                        </a>
                    <?php endforeach; ?>
                </div>
    ...
    public function append_links_to_content($content)
    {
        ...
        if ($post->post_type === 'program') {
            ...
            if (!empty($cities)) {
                ...
                $content .= '<div class="wimper-combo-links" style="margin-top: 2rem; padding: 1.5rem; background: #f8f8f8; border-radius: 8px;">';
...
new wimper_Combo_Internal_Links();
