...
class wimper_Combo_Page_Generator
{
    const REWRITE_TAG = 'wimper_combo';
    ...
    public function __construct()
    {
        ...
        if (!get_option('wimper_combo_sitemap_flush_Check')) {
            add_action('init', function () {
                flush_rewrite_rules();
                update_option('wimper_combo_sitemap_flush_Check', true);
            }, 99);
        }
    }
    ...
    {
        $provider = new wimper_Combo_Sitemap_Provider();
    ...
    public function add_rewrite_rules()
    {
        ...
        add_rewrite_rule(
            '^sitemap-combos\.xml$',
            'index.php?wimper_combo_sitemap=1',
            'top'
        );
    }
    ...
    public function add_query_vars($vars)
    {
        ...
        $vars[] = 'combo_city';
        $vars[] = 'combo_state';
        $vars[] = 'wimper_combo_sitemap'; // Query var for sitemap
        return $vars;
    }
    ...
    public function handle_combo_page()
    {
        ...
        $seo_title = "{$program_title} in {$city_name}, {$state} | WIMPER";

        // Truncate if too long (max 60 chars)
        if (strlen($seo_title) > 60) {
            $seo_title = "{$program_title} in {$city_name} | WIMPER";
        }
        if (strlen($seo_title) > 60) {
    ...
    public function generate_combo_meta_description($program, $city_name, $state, $age_range)
    {
        ...
        $templates = [
            ...
            'ga-pre-k' => "Free GA Pre-K in {$city_name} for 4-year-olds. State-funded, kindergarten-ready curriculum at WIMPER. Limited spots—enroll now!",
        ...
        if (isset($templates[$program_slug])) {
            ...
            $description .= ". Quality early learning with caring teachers at WIMPER Early Learning. Schedule a tour today!";
        }
    ...
    public function get_combo_page_html($program, $city_slug, $state, $location)
    {
        ...
        // This is stored in options table via wimper_Combo_Page_Data class
        $combo_custom_data = wimper_Combo_Page_Data::get($program->post_name, $city_slug, $state);
        ...
        if (!empty($combo_custom_data['custom_intro'])) {
            ...
            $intro_text = sprintf(
                'Searching for the best %s near %s? At WIMPER %s, we combine the safety you need with the enriching curriculum your child deserves.',
        ...
        ob_start();
        ?>
                <main class="combo-page bg-wimper-cream">
            
                    <!-- Hero Section -->
                    <section class="relative pt-20 pb-24 bg-white overflow-hidden">
                        <div class="absolute top-0 right-0 w-2/3 h-full bg-gradient-to-l from-wimper-blue/5 to-transparent -z-10"></div>
                        <div class="max-w-7xl mx-auto px-4 lg:px-6 grid lg:grid-cols-2 gap-12 items-center">
                            <div>
                                <?php if ($age_range): ?>
                                    <div class="inline-flex items-center gap-2 bg-wimper-blue/10 text-wimper-blue px-4 py-1.5 rounded-full text-[11px] font-bold uppercase tracking-widest mb-6">
                                        Now Enrolling: <?php echo esc_html($age_range); ?>
                                    </div>
                                <?php endif; ?>
                        
                                <h1 class="font-serif text-4xl md:text-5xl lg:text-6xl text-wimper-ink mb-6 leading-tight">
                                    Premier <?php echo esc_html($program->post_title); ?> in 
                                    <span class="italic text-wimper-blue"><?php echo esc_html($city_name); ?>, <?php echo esc_html($state); ?>.</span>
                                </h1>
                        
                                <p class="text-lg text-wimper-ink/70 mb-8 leading-relaxed">
                                    <?php echo wp_kses_post($intro_text); ?>
                                </p>
                        
                                <div class="flex flex-wrap gap-4">
                                    <a href="#tour" class="inline-flex items-center justify-center px-8 py-4 rounded-full bg-wimper-blue text-white text-xs font-bold uppercase tracking-[0.2em] shadow-soft hover:bg-wimper-ink transition-all">
                                        Schedule Visit
                                    </a>
                                    <?php
                                    ...
                                    <?php if ($show_loc_details && $loc_address): ?>
                                        <span class="flex items-center gap-2 text-sm font-bold text-wimper-ink/60 px-4 py-4">
                                            <svg class="w-4 h-4 text-wimper-red" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z" clip-rule="evenodd"/></svg>
                                            <?php echo esc_html($loc_address); ?>
                                        </span>
                                    <?php else: ?>
                                        <span class="flex items-center gap-2 text-sm font-bold text-wimper-ink/60 px-4 py-4">
                                            <svg class="w-4 h-4 text-wimper-blue" fill="currentColor" viewBox="0 0 20 20"><path d="M9 6a3 3 0 11-6 0 3 3 0 016 0zM17 6a3 3 0 11-6 0 3 3 0 016 0zM12.93 17c.046-.327.07-.66.07-1a6.97 6.97 0 00-1.5-4.33A5 5 0 0119 16v1h-6.07zM6 11a5 5 0 015 5v1H1v-1a5 5 0 015-5z"/></svg>
                            ...
                            <div class="relative h-[400px] lg:h-[450px] rounded-[2rem] lg:rounded-[3rem] overflow-hidden shadow-2xl border-4 border-white">
                                <?php if ($program_image): ?>
                                        <img src="<?php echo esc_url($program_image); ?>" 
                                             ...
                                             loading="eager">
                                <?php else: ?>
                                        <div class="w-full h-full bg-gradient-to-br from-wimper-blue/20 to-wimper-red/20 flex items-center justify-center">
                    ...
                    <!-- Why Choose Section -->
                    <section class="py-20 bg-wimper-cream">
                        <div class="max-w-7xl mx-auto px-4 lg:px-6">
                            <div class="text-center mb-16 max-w-3xl mx-auto">
                                <h2 class="text-3xl md:text-4xl font-serif font-bold text-wimper-ink mb-4">
                                    Why <?php echo esc_html($city_name); ?> Parents Choose Our <?php echo esc_html($program->post_title); ?>
                                </h2>
                                <p class="text-wimper-ink/60">
                            ...
                            <div class="grid md:grid-cols-3 gap-8">
                                ...
                                <div class="bg-white p-8 rounded-3xl shadow-sm border border-wimper-ink/5">
                                    <div class="w-12 h-12 bg-wimper-red/10 text-wimper-red rounded-xl flex items-center justify-center text-xl mb-4">
                                    ...
                                    <p class="text-sm text-wimper-ink/70">Our <?php echo esc_html($city_name); ?> campus maintains strict teacher-to-student ratios, ensuring your child gets the individual attention they need.</p>
                                </div>
                        
                                <!-- Benefit 2: WIMPER Creative Curriculum -->
                                <div class="bg-white p-8 rounded-3xl shadow-sm border border-wimper-ink/5">
                                    <div class="w-12 h-12 bg-wimper-yellow/10 text-wimper-yellow rounded-xl flex items-center justify-center text-xl mb-4">
                                        <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>
                                    </div>
                                    <h3 class="font-bold text-xl mb-2">WIMPER Creative Curriculum™ Curriculum</h3>
                                    <p class="text-sm text-wimper-ink/70">Specifically designed for <?php echo esc_html($age_range ?: 'early learners'); ?>, our curriculum balances play-based learning with school readiness.</p>
                                </div>
                        
                                <!-- Benefit 3: Real-Time Updates -->
                                <div class="bg-white p-8 rounded-3xl shadow-sm border border-wimper-ink/5">
                                    <div class="w-12 h-12 bg-wimper-green/10 text-wimper-green rounded-xl flex items-center justify-center text-xl mb-4">
                                    ...
                                    <p class="text-sm text-wimper-ink/70">Parents in <?php echo esc_html($city_name); ?> love our app. Get photos and updates throughout the workday straight to your phone.</p>
                                </div>
                            </div>
                        </div>
                    </section>
                    ...
                    <section class="py-20 bg-white border-y border-wimper-ink/5">
                        <div class="max-w-4xl mx-auto px-4 text-center">
                            ...
                            <p class="text-lg text-wimper-ink/70 leading-relaxed">
                                <?php if ($major_road): ?>
                    ...
                    <section id="tour" class="py-24 bg-wimper-blueDark text-white">
                        <div class="max-w-7xl mx-auto px-4 text-center">
                            ...
                            if (!empty($nearby_ids) && is_array($nearby_ids)):
                                ...
                                ?>
                                    <h2 class="font-serif text-3xl md:text-4xl font-bold mb-6">WIMPER Locations Serving <?php echo esc_html($city_name); ?></h2>
                                    ...
                                    <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8 text-left">
                                        ...
                                        while ($grid_query->have_posts()):
                                            ...
                                            ?>
                                            <div class="group p-6 rounded-3xl bg-white text-wimper-ink shadow-2xl transition-all hover:-translate-y-1">
                                                <div class="h-48 rounded-2xl bg-wimper-cream mb-6 overflow-hidden relative">
                                                    <img src="<?php echo esc_url($grid_image); ?>" class="w-full h-full object-cover" alt="<?php the_title_attribute(); ?>">
                                                    <div class="absolute top-4 right-4 bg-white px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide shadow-sm text-wimper-ink">
                                                ...
                                                <div class="mt-auto">
                                                    <a href="<?php the_permalink(); ?>" class="block w-full py-3 bg-wimper-blue text-white text-center rounded-xl text-xs font-bold uppercase tracking-widest hover:bg-wimper-ink transition-colors">
                                                        View Campus
                                                    </a>
                                                </div>
                                            </div>
                            ...
                            <?php else: ?>
                                    ...
                                    <div class="max-w-3xl mx-auto">
                                        ...
                                        <div class="bg-white p-8 rounded-[2rem] text-left shadow-2xl">
                                            <?php echo do_shortcode('[wimper_tour_form]'); ?>
                                        </div>
                                    </div>
                            <?php endif; ?>
                        </div>
                    ...
                    <!-- FAQ Section -->
                    <section class="py-24 bg-white border-t border-wimper-ink/5">
                        <div class="max-w-4xl mx-auto px-4">
                            <h2 class="font-serif text-3xl md:text-4xl font-bold text-center text-wimper-ink mb-12">Common Questions</h2>
                    
                            <div class="space-y-4">
                                <details class="group bg-wimper-cream rounded-2xl p-6 shadow-sm border border-wimper-ink/5 cursor-pointer">
                                    <summary class="flex items-center justify-between font-bold text-wimper-ink list-none">
                                        <span>What are the tuition rates for <?php echo esc_html($program->post_title); ?> in <?php echo esc_html($city_name); ?>?</span>
                                        <span class="text-wimper-blue group-open:rotate-180 transition-transform">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
                                        </span>
                                    </summary>
                                    <p class="mt-3 text-sm text-wimper-ink/70 leading-relaxed">
    ...
    public function output_schema($program, $city_name, $state, $location, $loc_address = '', $loc_zip = '', $age_range = '')
    {
        $schema = [
            ...
            'provider' => [
                '@type' => 'Preschool',
                'name' => 'WIMPER Early Learning Academy - ' . $city_name,
                'address' => [
                    '@type' => 'PostalAddress',
    ...
    {
        if (get_query_var('wimper_combo_sitemap') == 1) {
            ...
            foreach ($combos as $combo) {
                ...
                $saved_data = wimper_Combo_Page_Data::get($program_slug, $city_slug, $state);
    ...
    public function render_admin_page() {
        ...
        foreach ($combos as &$combo) {
            if (class_exists('wimper_Combo_Page_Data')) {
                $combo['data'] = wimper_Combo_Page_Data::get(
        ...
        $base_url = admin_url('admin.php?page=wimper-auto-pages');
        $auto_publish = get_option('wimper_combo_auto_publish', false);
        $nonce = wp_create_nonce('wimper_combo_ai');
        ?>
        <div class="wrap">
            <h1>🚀 WIMPER Auto-Generated Pages</h1>
            ...
            <div class="combo-stats" style="display: flex; gap: 20px; margin: 20px 0;">
                ...
                <div class="stat-box" style="background: #fff; border: 1px solid #ccc; padding: 15px 25px; border-radius: 8px; text-align: center;">
                    <div style="font-size: 28px; font-weight: bold; color: #00a32a;">
                        <?php echo count(array_filter($combos, fn($c) => ($c['data']['status'] ?? 'auto') === 'published')); ?>
                    </div>
                    <div style="color: #666;">Published</div>
                </div>
                <div class="stat-box" style="background: #fff; border: 1px solid #ccc; padding: 15px 25px; border-radius: 8px; text-align: center;">
                    <div style="font-size: 28px; font-weight: bold; color: #dba617;">
                        <?php echo count(array_filter($combos, fn($c) => ($c['data']['ai_generated'] ?? false))); ?>
                    </div>
            ...
            <div class="tablenav top" style="margin-bottom: 15px;">
                ...
                <div class="alignright" style="display: flex; gap: 10px; align-items: center;">
                    <form method="get" style="display: inline;">
                        <input type="hidden" name="page" value="wimper-auto-pages">
                        <label>Show: 
                    ...
                    <tr data-program="<?php echo esc_attr($program_slug); ?>" 
                        ...
                        <td>
                            <span style="color: <?php echo $status_color; ?>; font-weight: 500;">
                                <?php 
                                if (class_exists('wimper_Combo_Page_Data')) {
                                    echo esc_html(wimper_Combo_Page_Data::get_status_label($status));
                                } else {
            ...
            <form method="post" action="options.php">
                <?php settings_fields('wimper_auto_pages'); ?>
                <table class="form-table">
                    <tr>
                        <th>Auto-Publish AI Content</th>
                        <td>
                            <label>
                                <input type="checkbox" name="wimper_combo_auto_publish" value="1" <?php checked($auto_publish); ?>>
                    ...
                    <tr>
                        <th>Manual Cities</th>
                        <td>
                            <textarea name="wimper_seo_manual_cities_raw" rows="5" class="large-text"><?php 
                                $manual = get_option('wimper_seo_manual_cities', []);
                                foreach ($manual as $m) {
        ...
        jQuery(function($) {
            ...
            $('.ai-generate-btn, #ai-generate-modal-btn').on('click', function() {
                ...
                $.post(ajaxurl, {
                    action: 'wimper_combo_ai_generate',
                    nonce: nonce,
                    program_slug: program,
                    city_slug: city,
                    state: state
            ...
            $('#do-bulk-action').on('click', function() {
                ...
                if (action === 'ai_generate') {
                    $.post(ajaxurl, {
                        action: 'wimper_combo_ai_bulk_generate',
                        nonce: nonce,
                        combos: combos
                    }, function(response) {
                    ...
                    $.post(ajaxurl, {
                        action: 'wimper_combo_bulk_status',
                        nonce: nonce,
                        combos: combos,
                        status: status
                    }, function(response) {
...
add_action('admin_init', function () {
    // Auto-publish toggle
    register_setting('wimper_auto_pages', 'wimper_combo_auto_publish');

    // Manual cities
    register_setting('wimper_auto_pages', 'wimper_seo_manual_cities_raw', function ($raw) {
        $cities = [];
        $lines = explode("\n", $raw);
        ...
        update_option('wimper_seo_manual_cities', $cities);
        return $raw;
    });
});

new wimper_Combo_Page_Generator();

/**
 * Custom Sitemap Provider for Combo Pages
 * Integration with Native WordPress Sitemaps
 */
class wimper_Combo_Sitemap_Provider extends WP_Sitemaps_Provider
{
