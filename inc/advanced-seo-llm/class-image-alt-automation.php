...
class kidazzle_Image_Alt_Automation
{
    ...
    public function inject_auto_alt_text($attr, $attachment, $size)
    {
        ...
        if ($post_type === 'location') {
            ...
            $auto_alt = sprintf(
                __('Childcare facility at %s in %s - Quality Rated learning environment', 'kidazzle'),
                $title,
                $city ? $city : 'Georgia'
            );
        } elseif ($post_type === 'program') {
            $auto_alt = sprintf(
                __('%s program for early childhood education - Hands-on learning activities', 'kidazzle'),
                $title
            );
        } else {
            // Generic fallback using site name
            $auto_alt = sprintf(
                __('%s - Early Childhood Education Center', 'kidazzle'),
                get_bloginfo('name')
            );
        }
